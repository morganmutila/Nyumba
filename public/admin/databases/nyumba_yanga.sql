...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`users` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `phone` (`phone` ASC) INVISIBLE,
  UNIQUE INDEX `username` (`username` ASC) INVISIBLE,
  INDEX `name` USING BTREE (`first_name`, `last_name`) INVISIBLE,
  INDEX `login` (`username` ASC, `email` ASC, `phone` ASC, `password` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`staff` (
  ...
  UNIQUE INDEX `phone` (`phone` ASC) INVISIBLE,
  INDEX `name` USING BTREE (`first_name`, `last_name`) INVISIBLE,
  INDEX `login` USING BTREE (`email`, `password`) INVISIBLE,
  INDEX `staff_group1_idx` (`group_id` ASC) INVISIBLE,
  INDEX `staff_city1_idx` (`city_id` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`property` (
  ...
  INDEX `added_on` (`added` ASC) INVISIBLE,
  INDEX `property_price` USING BTREE (`price`, `market`) INVISIBLE,
  INDEX `market_type` USING BTREE (`market`) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`property_feature` (
  ...
  `last_update` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `feature_name` (`feature` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`amenities` (
  ...
  INDEX `property_id_idx` (`property_id` ASC) INVISIBLE,
  INDEX `feature_id_idx` (`feature_id` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`property_image` (
  ...
  `last_update` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `property_photo` (`property_id` ASC) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`saved_property` (
  ...
  PRIMARY KEY (`id`),
  INDEX `user_property_id` USING BTREE (`property_id`, `user_id`) INVISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`reported_property` (
  ...
  `property_id` INT(11) UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_reported_property_property1_idx` (`property_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `nyumba_yanga`.`saved_search` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_saved_search_users1_idx` (`users_id` ASC) VISIBLE,
